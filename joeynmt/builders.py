...
def build_gradient_clipper(config: dict) -> Optional[Callable]:
    ...
    if "clip_grad_val" not in config and "clip_grad_norm" not in config:
        return None

    if "clip_grad_val" in config:
        clip_value = config["clip_grad_val"]
        clip_grad_fun = partial(clip_grad_value_, clip_value=clip_value)
    else:
        max_norm = config["clip_grad_norm"]
        clip_grad_fun = partial(clip_grad_norm_, max_norm=max_norm)
...
def build_scheduler(config: dict, optimizer: Optimizer, scheduler_mode: str,
    ...
    """
    if "scheduling" not in config or not config["scheduling"]:
        return None, None

    schedule_type = config["scheduling"].lower()
    step_types = {
        "plateau": "validation",
        "decaying": "epoch",
        "exponential": "epoch",
    ...
    assert schedule_type in step_types
    scheduler_step_at = step_types[schedule_type]
    if schedule_type == "plateau":
        ...
        scheduler = ReduceLROnPlateau(
            ...
            factor=factor,
            patience=patience)
    elif schedule_type == "decaying":
        step_size = config.get("decaying_step_size", 1)
        scheduler = StepLR(optimizer=optimizer, step_size=step_size)
        ...
        scheduler = NoamScheduler(
            hidden_size=hidden_size, factor=factor,
